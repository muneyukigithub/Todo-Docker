...
import { Box, Button, Grid, List, ListItem, ListItemButton, ListItemIcon, Menu, MenuItem } from '@mui/material';
    ...
    <SidebarDiv>
      ...
      <List sx={{ width: '100%' }}>
        {taskCardList.map((taskcard, i) => {
          return <Grid key={taskcard.tasklist_id} container spacing={0} alignItems="center">
            <Grid item xs={8}>
              <ListItem key={taskcard.tasklist_id} disablePadding onClick={() => handleSelectTaskCard(i)}>
            ...
            <Grid item xs={2}>
              <IconButton edge="end" aria-label="delete" onClick={() => handleDeleteTaskCard(taskcard)} >
                <DeleteIcon />
              </IconButton>
            </Grid>
          </Grid>
      ...
      <Menu
        ...
        onClose={handleClose}
      >
        <MenuItem onClick={() => handleAction(() => handleChangeTaskStatus(menuValue, "waiting"))}>{"未着手にする"}</MenuItem>
        <MenuItem onClick={() => handleAction(() => handleChangeTaskStatus(menuValue, "inprogress"))}>{"作業中にする"}</MenuItem>
        <MenuItem onClick={() => handleAction(() => handleChangeTaskStatus(menuValue, "end"))}>{"完了にする"}</MenuItem>
      </Menu>
    </SidebarDiv >
  )
}
